...
class MainLive_RecordingPage extends Page<ILiveRecordingPageParams> {
        ...
        this.remote.getLiveList(that.liveName, that.livePageSize, that.liveCurrent)
         .then((res) => {
             ...
             if (resultCode === "0") {
                 ...
                 console.warn('livePager: ', that.livePager);
             }
         });
    ...
    recordPaging(total) {
        ...
        // 大于十页部分显示
            ...
            // 当前页号是最后5页之一
            else if (that.recordCurrent > total - 4) {
                return [
                    {type: 0, value: 1, isCurrent: false},
                    {type: 1}
                ]
            ...
            // 其它情况
                ...
                return [
                    ...
                    .concat([that.recordCurrent - 2, that.recordCurrent - 1, that.recordCurrent, that.recordCurrent + 1, that.recordCurrent + 2].map(x => ({
                        type: 0,
                        value: x,
                        isCurrent: x === that.recordCurrent
                    })))
