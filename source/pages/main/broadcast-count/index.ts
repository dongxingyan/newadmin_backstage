...
class MainBroadcast_CountPage extends Page<IBroadcastCountPageParams> {
    ...
    liveUserZoneChart;
    liveUserDeviceChart;
    liveBrowserVersionChart;
    liveFlowChart;
    timeReg;
    liveList;
    isShowOfSelect = true;
    $liveListSelect = $('#J-live-list-select');
    showUserZone : boolean;
    ...
    getCountData (startTime, endTime, liveId) {
        ...
        self.remote.getLiveCountData (requestData)
            .then((res) => {
                ...
                let resDataList = result.data;
                if (resCode === "0") {
                  if (resDataList.timeline) {
                    document.querySelector('#concurrent-chart').classList.remove('uc-hide');
                    document.querySelector('#concurrent-chart+div').classList.add('uc-hide');
                    document.querySelector('#flow-chart').classList.remove('uc-hide');
                    document.querySelector('#flow-chart+div').classList.add('uc-hide');
                    // 渲染并发数统计图表
                    self.renderConcurrentChart(resDataList.timeline);
                    // 渲染流量统计图表
                    self.renderFlowChart(resDataList.timeline);
                  } else {
                    document.querySelector('#concurrent-chart').classList.add('uc-hide');
                    document.querySelector('#concurrent-chart+div').classList.remove('uc-hide');
                    document.querySelector('#flow-chart').classList.add('uc-hide');
                    document.querySelector('#flow-chart+div').classList.remove('uc-hide');
                  }

                    if (resDataList.region) {
                      // 渲染用户分布地区统计图表
                      this.showUserZone = true;
                      document.querySelector('#user-zone-chart').classList.remove('uc-hide');
                      document.querySelector('#user-zone-chart+div').classList.add('uc-hide');
                      self.renderUserZoneChart(resDataList.region);
                    } else {
                      document.querySelector('#user-zone-chart').classList.add('uc-hide');
                      document.querySelector('#user-zone-chart+div').classList.remove('uc-hide');
                      this.showUserZone = false;
                    }

                    if (resDataList.device) {
                      // 渲染用户设备统计图表 user-device-chart
                      document.querySelector('#user-device-chart').classList.remove('uc-hide');
                      document.querySelector('#user-device-chart+div').classList.add('uc-hide');
                      self.renderUserDeviceChart(resDataList.device);
                    } else {
                      document.querySelector('#user-device-chart').classList.add('uc-hide');
                      document.querySelector('#user-device-chart+div').classList.remove('uc-hide');
                    }
                    // self.renderUserDeviceChart();

                    if (resDataList.browser) {
                      // 渲染浏览器版本统计图表
                      document.querySelector('#browser-version-chart').classList.remove('uc-hide');
                      document.querySelector('#browser-version-chart+div').classList.add('uc-hide');
                      self.renderBrowserVersionChart(resDataList.browser);
                    } else {
                      document.querySelector('#browser-version-chart').classList.add('uc-hide');
                      document.querySelector('#browser-version-chart+div').classList.remove('uc-hide');
                    }
    ...
    renderConcurrentChart (data) {
        ...
        let option = {
            ...
            title: {
                text: this.translate.instant('ADMIN_CONCURRENCY'),
                x: 'center'
            },
            tooltip: {
                show: true,
            ...
            series : [
                {
                    "name": this.translate.instant('ADMIN_CONCURRENT'),
                    "type": "bar",
                    "label": {
                        normal: {
                            show: true,
    ...
    renderUserZoneChart (data) {
        ...
        let userZoneOption = {
            ...
            title: {
                text: this.translate.instant('ADMIN_USER_AREA'),
                x: 'center'
            },
            tooltip: {
                show: true,
            ...
            series : [
                {
                    "size": ['60%', '60%'],
                    "name": this.translate.instant('ADMIN_PEOPLE_COUNT'),
                    "type": "bar",
                    "label": {
                        normal: {
                            show: true,
    ...
    renderUserDeviceChart (data) {
        ...
        let userDeviceOption = {
            ...
            title : {
                text: this.translate.instant('ADMIN_EQUIPMENT'),
                'x': 'center'
            },
            tooltip : {
                trigger: 'item',
            ...
            series : [
                {
                    name: this.translate.instant('ADMIN_SOURCE'),
                    type: 'pie',
                    radius : '70%',
                    center: ['50%', '50%'],
                    data : seriesOfData,
    ...
    renderBrowserVersionChart (data) {
        ...
        let browserVersionOption = {
            ...
            title: {
                text: this.translate.instant('ADMIN_BROWSER_VERSION'),
                x: 'center'
            },
            tooltip: {
                show: true,
            ...
            series : [
                {
                    "name": this.translate.instant('ADMIN_NUMBER2'),
                    "type": "bar",
                    "label": {
                        normal: {
                            show: true,
    ...
    renderFlowChart (data) {
        ...
        let flowOption = {
            ...
            title: {
                text: this.translate.instant('ADMIN_FLOW'),
                x: 'center'
            },
            tooltip: {
                show: true,
            ...
            series : [
                {
                    "name": this.translate.instant('ADMIN_FLOW'),
                    "type": "line",
                    "label": {
                        normal: {
                            show: true,
    ...
    outputChartsService (startTime, endTime, liveId) {
        ...
        if (liveId || liveId !== '0') {
            requestData['liveId'] = liveId;
        }
        // 获取导出报表的url，并导出报表
        this.remote.getLiveURLOfOutputChart (requestData)
            .then((res) => {
                let result = res.data;
                let resCode = result.code;
