...
class MainAccount_SafetyPage extends Page<IAccountSafetyPageParams> {
                ...
                if(!value){
                    ...
                    await this.remote.checkEmail({email: value}).then(res => {
                        if(res.data.code === '0'){
                            ...
                            callback(new Error(this.translate.instant('ADMIN_ERROR_TIP28')));
                        } else {
                            callback(new Error(this.translate.instant('ADMIN_ERROR_TIP31')));
                        }
                    })
                }
            },
    ...
    sendEmailCode() {
        // 验证邮箱账号是否填写
        this.validateField(this.rules, 'email', (valid) => {
            if (valid) {

                // 主动发送邮件
                this.remote.sendEmailCode({email: this.email}).then(res => {
                    if (res.data.code === '0') {
                        // 验证码发送ok
                        this.count--;
        ...
        this.validate(this.rules, (valid) => {
            if (valid) {
                ...
                this.remote.alterAndBindEmail(reqData).then(res => {
                    if (res.data.code === '0') {
                        this.rootScope.toggleInfoModal(1, this.translate.instant('ADMIN_SUCCESS9'))
                        setTimeout(() => {
